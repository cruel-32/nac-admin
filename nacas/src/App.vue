...
export default class App extends Vue {
  ...
  mounted(){
    ...
    this.$validator.localize('en', {
      custom : {
        title: {
          required : (field:any) => `${field} 모임 제목을 적으세요`,
          max: (field:any,count:number) => `${field}필드는 ${count} 자를 초과할 수 없습니다`
        },
        place: {
          required : (field:any) => `${field} 모임 장소를 선택하세요`,
        },
        contents: {
          required : (field:any) => `${field} 모임 컨텐츠를 선택하세요`,
          min : (field:any, count:number) => `모임 ${field}를 ${count}개 이상 선택하세요`
        },
        date : {
  ...
  signOut():void {
    DefaultApi.Firebase.auth.signOut().then(()=>{
